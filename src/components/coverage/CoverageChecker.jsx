...
const MAP_STYLES = [
  { id: "osm",  label: "Street",  url: "https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png" },
...
export default function CoverageChecker({ onClose }) {
  ...
  useEffect(() => {
    ...
    const unsub = base44.entities.FibreProject.subscribe((event) => {
      if (event.type === "update" && ["live", "billed"].includes(event.data?.status)) fetchProjects();
    });
    return unsub;
  }, []);
      ...
      <div className="relative w-full max-w-5xl rounded-2xl overflow-hidden flex flex-col"
        ...
        <div className="flex items-center justify-between px-5 py-3.5 flex-shrink-0"
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="flex items-center gap-1.5 px-2 py-1 rounded-lg text-[10px] font-bold ml-2"
              style={{ background: "rgba(16,185,129,0.1)", border: "1px solid rgba(16,185,129,0.2)", color: "#10b981" }}>
              <span className="w-1.5 h-1.5 rounded-full bg-emerald-400 animate-pulse" />
              Live Sync
              {lastSync && <span className="ml-1 opacity-60">{lastSync.toLocaleTimeString()}</span>}
            </div>
          </div>
        ...
        <div className="flex flex-1 overflow-hidden">
          ...
          {/* Sidebar */}
          <div className="flex flex-col w-60 flex-shrink-0 overflow-y-auto"
            ...
            <div className="p-3" style={{ borderBottom: "1px solid rgba(155,143,239,0.1)" }}>
              <p className="text-[9px] font-black uppercase tracking-[0.2em] mb-2" style={{ color: "rgba(196,188,247,0.4)" }}>Search Address</p>
            ...
            {/* Map style */}
            <div className="p-3" style={{ borderBottom: "1px solid rgba(155,143,239,0.1)" }}>
              <button className="w-full flex items-center justify-between mb-1" onClick={() => setShowStylePicker(v => !v)}>
                <div className="flex items-center gap-2">
                ...
                {showStylePicker ? <ChevronUp className="w-3 h-3" style={{ color: "#9b8fef" }} /> : <ChevronDown className="w-3 h-3" style={{ color: "#9b8fef" }} />}
              </button>
              {showStylePicker && (
            ...
            {/* Layers */}
            <div className="p-3 flex-1" style={{ borderBottom: "1px solid rgba(155,143,239,0.1)" }}>
              <button className="w-full flex items-center justify-between mb-2" onClick={() => setLegendOpen(v => !v)}>
                <div className="flex items-center gap-2">
                  <Info className="w-3.5 h-3.5" style={{ color: "#9b8fef" }} />
              ...
              {legendOpen && (
                <div className="space-y-1.5">
                  <button onClick={() => setShowTouchNet(v => !v)}
                    ...
                    <div className="flex items-center gap-2">
                      <span className="w-3 h-3 rounded-full" style={{ background: "#9b8fef", boxShadow: "0 0 6px rgba(155,143,239,0.7)" }} />
            ...
            {selectedProject && (
              <div className="p-3" style={{ borderTop: "1px solid rgba(155,143,239,0.12)" }}>
                <div className="flex items-center justify-between mb-2">
                  <p className="text-[9px] font-black uppercase tracking-[0.2em]" style={{ color: "rgba(196,188,247,0.4)" }}>Selected Zone</p>
                ...
                <div className="rounded-xl p-3" style={{ background: "rgba(155,143,239,0.08)", border: "1px solid rgba(155,143,239,0.15)" }}>
                  ...
                  {selectedProject.service_plan && <p className="text-[10px] mt-1 font-mono" style={{ color: "#9b8fef" }}>{selectedProject.service_plan}</p>}
                  {selectedProject.customer_name && <p className="text-[10px] mt-1" style={{ color: "rgba(196,188,247,0.4)" }}>{selectedProject.customer_name}</p>}
                </div>
              </div>
            )}

            {/* Stats */}
            <div className="p-3 mt-auto" style={{ borderTop: "1px solid rgba(155,143,239,0.1)" }}>
                ...
                <Circle key={project.id}
                  ...
                  <Popup>
                    <div style={{ minWidth: 160 }}>
                      ...
                      {project.service_plan && <p style={{ fontSize: 11, color: "#6366f1", marginTop: 2 }}>{project.service_plan}</p>}
                    </div>
                  </Popup>
                </Circle>
              ))}
