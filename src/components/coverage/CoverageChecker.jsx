    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4"
      ...
      <div className="w-full max-w-md rounded-3xl overflow-hidden relative"
        style={{
          ...
          boxShadow: "0 32px 80px rgba(167,139,250,0.3), 0 8px 32px rgba(0,0,0,0.7)",
        }}
        onClick={e => e.stopPropagation()}>

        <div className="h-[3px]" style={{ background: "linear-gradient(90deg,#a78bfa,#22d3ee,#4ade80,transparent)" }} />

        <div className="flex items-center justify-between px-6 pt-5 pb-4"
          style={{ borderBottom: "1px solid rgba(167,139,250,0.14)" }}>
          <div className="flex items-center gap-3">
            <div className="w-9 h-9 rounded-xl flex items-center justify-center"
              style={{ background: "linear-gradient(135deg,rgba(34,211,238,0.2),rgba(167,139,250,0.15))", border: "1px solid rgba(34,211,238,0.3)" }}>
              <MapPin className="w-4 h-4" style={{ color: "#22d3ee" }} />
            </div>
            <div>
              <p className="text-[15px] font-black" style={{ color: "#f5f0ff" }}>Quick Coverage Check</p>
              <p className="text-[10px] mono" style={{ color: "rgba(221,214,254,0.45)" }}>South Africa · Multi-provider</p>
            </div>
          </div>
        ...
        <div className="px-6 py-5 space-y-4">
          <div className="flex gap-2">
            <div className="relative flex-1">
              <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4" style={{ color: "rgba(221,214,254,0.4)" }} />
              <input
                className="w-full pl-10 pr-4 py-3 rounded-xl text-[13px] outline-none transition-all"
                style={{ background: "rgba(167,139,250,0.09)", border: "1px solid rgba(167,139,250,0.28)", color: "#ede8ff" }}
            ...
            <button onClick={check} disabled={loading || !query.trim()}
              ...
              style={{ background: "linear-gradient(135deg,#8b5cf6,#a78bfa)", boxShadow: "0 4px 18px rgba(167,139,250,0.45)" }}>
          ...
          {result && (
            <div className="rounded-2xl overflow-hidden"
              style={{
                background: result.covered ? "rgba(74,222,128,0.06)" : "rgba(248,113,113,0.06)",
                border: `1px solid ${result.covered ? "rgba(74,222,128,0.3)" : "rgba(248,113,113,0.3)"}`,
              }}>
              <div className="h-[2px]"
                style={{ background: result.covered ? "linear-gradient(90deg,#4ade80,#22d3ee,transparent)" : "linear-gradient(90deg,#f87171,#f97316,transparent)" }} />
              <div className="p-4 flex gap-3">
                <div className="w-9 h-9 rounded-xl flex items-center justify-center flex-shrink-0"
                  style={{ background: result.covered ? "rgba(74,222,128,0.15)" : "rgba(248,113,113,0.15)" }}>
                  {result.covered
                    ? <CheckCircle2 className="w-5 h-5" style={{ color: "#4ade80" }} />
                    : <XCircle className="w-5 h-5" style={{ color: "#f87171" }} />}
                </div>
                <div className="flex-1 min-w-0">
                  <p className="text-[14px] font-black" style={{ color: result.covered ? "#4ade80" : "#f87171" }}>
                    {result.covered ? "Coverage Available!" : "Not Yet Covered"}
                  </p>
                  {result.area && (
                    <p className="text-[11px] mono mt-0.5" style={{ color: "rgba(221,214,254,0.55)" }}>{result.area}</p>
                  )}
                  <p className="text-[12px] mt-1.5 leading-relaxed" style={{ color: "rgba(237,232,255,0.75)" }}>{result.message}</p>
                    ...
                    <div className="flex flex-wrap gap-1.5 mt-2">
                      {result.providers_likely.map(p => (
                        <span key={p} className="text-[10px] font-bold px-2 py-0.5 rounded-lg"
                          style={{ background: "rgba(167,139,250,0.14)", color: "#ddd6fe", border: "1px solid rgba(167,139,250,0.28)" }}>
                          {p}
                        </span>
                      ))}
                    </div>
                  ...
                  {result.estimate && (
                    <p className="text-[11px] mt-1.5 font-semibold" style={{ color: "rgba(221,214,254,0.65)" }}>{result.estimate}</p>
                  )}
                </div>
              </div>
              <div className="px-4 pb-4">
                <a href="/CoverageCheck"
                  ...
                  style={{ background: "linear-gradient(135deg,#8b5cf6,#a78bfa)", boxShadow: "0 4px 14px rgba(167,139,250,0.4)" }}>
