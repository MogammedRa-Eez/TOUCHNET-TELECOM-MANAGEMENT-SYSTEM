          ...
          <div>
            <div className="flex items-center gap-2 mb-1 flex-wrap">
              <div className="w-9 h-9 rounded-xl flex items-center justify-center" style={{ background: "rgba(0,180,180,0.15)", border: "1px solid rgba(0,180,180,0.4)", boxShadow: "0 0 14px rgba(0,180,180,0.2)" }}>
                <UserCog className="w-4.5 h-4.5" style={{ color: "#00b4b4" }} />
              </div>
              <h1 className="text-xl font-black tracking-tight glow-text-navy" style={{ fontFamily: "'Space Grotesk', sans-serif" }}>Employee Directory</h1>
            ...
            <p className="text-[11px] mono pl-11" style={{ color: "rgba(255,255,255,0.35)" }}>
              {visibleEmployees.length} staff · <span style={{ color: "#10b981" }}>{visibleEmployees.filter(e => e.status === "active").length} active</span> · <span style={{ color: "#f59e0b" }}>{visibleEmployees.filter(e => e.status === "on_leave").length} on leave</span>
            </p>
          </div>
          {isAdmin && (
