...
function _getDistKm(lat1, lng1, lat2, lng2) {
...
function MapFlyTo({ position }) {
  const map = useMap();
  React.useEffect(() => { if (position) map.flyTo(position, 14, { duration: 1.2 }); }, [position]);
  return null;
}

function CoverageChecker({ onClose }) {
  ...
  const handleSearch = async () => {
    ...
    const res = await fetch(`https://nominatim.openstreetmap.org/search?format=json&q=${encodeURIComponent(query+", South Africa")}&limit=1`, { headers: { "Accept-Language": "en" } });
    const data = await res.json();
    if (!data.length) { setResult({ found: false }); setSearching(false); return; }
    const lat = parseFloat(data[0].lat), lng = parseFloat(data[0].lon);
    setPin([lat, lng]);
    const covered = COVERAGE_ZONES.filter(z => _getDistKm(lat, lng, z.lat, z.lng)*1000 <= z.radius);
    const allProviders = [...new Set(covered.flatMap(c => c.providers))];
    const nearest = COVERAGE_ZONES.reduce((a,z) => { const d=_getDistKm(lat,lng,z.lat,z.lng); return d<a.d?{z,d}:a; }, {z:null,d:Infinity});
  ...
  return (
    <div className="fixed inset-0 flex items-center justify-center p-4" style={{ background: "rgba(0,0,0,0.55)", backdropFilter: "blur(6px)", zIndex: 70000 }}>
      <div className="flex flex-col w-full max-w-4xl rounded-3xl overflow-hidden shadow-2xl" style={{ background: "white", maxHeight: "90vh" }}>
        <div className="flex items-center justify-between px-6 py-4 flex-shrink-0" style={{ background: "linear-gradient(135deg,#0891b2,#06b6d4)" }}>
          ...
          <button onClick={onClose} className="w-8 h-8 flex items-center justify-center rounded-xl hover:bg-white/20">
            <X className="w-4 h-4 text-white" />
          </button>
        </div>
        <div className="flex gap-2 px-5 py-3 flex-shrink-0" style={{ borderBottom: "1px solid rgba(226,232,240,0.8)", background: "#f8fafc" }}>
        ...
        {result && (
          <div className="flex-shrink-0 px-5 py-2.5" style={{ background: result.inCoverage?"rgba(16,185,129,0.06)":result.found?"rgba(239,68,68,0.06)":"rgba(245,158,11,0.06)", borderBottom:"1px solid rgba(226,232,240,0.6)" }}>
            {!result.found ? <p className="text-[12px] font-semibold text-amber-700">Address not found. Try a more specific address.</p>
            : result.inCoverage ? <p className="text-[13px] font-bold text-emerald-700">✓ Fibre Available — {result.allProviders.join(", ").toUpperCase()}</p>
        ...
        <div className="flex-1 min-h-0" style={{ minHeight: 360 }}>
          <MapContainer center={[-26.0, 28.05]} zoom={10} style={{ height: "100%", width: "100%", minHeight: 360 }} scrollWheelZoom>
            <TileLayer url="https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png" attribution='&copy; OpenStreetMap contributors' />
            {pin && <MapFlyTo position={pin} />}
            {visibleZones.map(zone => {
              const p = filter!=="all" ? FIBRE_PROVIDERS.find(x=>x.id===filter) : FIBRE_PROVIDERS.find(x=>zone.providers.includes(x.id)&&x.id!=="all");
              const color = p?.color || "#6366f1";
              return (
                <Circle key={zone.id} center={[zone.lat,zone.lng]} radius={zone.radius} pathOptions={{ color, fillColor:color, fillOpacity:0.12, weight:1.5 }}>
                  <Popup><strong>{zone.name}</strong><br/>{zone.providers.map(pid=>{const pr=FIBRE_PROVIDERS.find(x=>x.id===pid);return pr?`• ${pr.label} — ${pr.speeds}`:null;}).filter(Boolean).join("\n")}</Popup>
                </Circle>
              );
            })}
